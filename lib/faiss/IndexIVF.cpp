...
void IndexIVF::search_with_centroids (idx_t n, const float *x, idx_t k, idx_t *cidx,
                         float* cdist, float *distances, idx_t *labels, long cmprob,
						 std::vector<RWLock *> &bucketRwLock,
                         bool toBlock) const
{	
	double t0 = getmillisecs();
    invlists->prefetch_lists (cidx, n * cmprob);
	//*prefetch_t = getmillisecs() - t0; 	

	//double t1 = getmillisecs();
    search_preassigned_parallel (n, x, k, cidx, cdist,
                        distances, labels, false, cmprob, bucketRwLock, toBlock);
	//*search_t = getmillisecs() - t1; 	

    indexIVF_stats.search_time += getmillisecs() - t0;
}

void IndexIVF::search_preassigned_parallel (idx_t n, const float *x, idx_t k,
                                   const idx_t *keys,
...
{
	int n_threads = 1;
    long nprobe = cmprob; //params ? params->nprobe : this->nprobe;
	...
	#pragma omp parallel if(do_parallel) reduction(+: nlistv, ndis, nheap)
    {
        ...
        /****************************************************
         * Only use Parallel Mode 1 (per invert list entry)
         ****************************************************/
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            for (size_t ik = 0; ik < nprobe; ik++) {
				// Read Lock Here
			    if (toBlock) {
					bucketRwLock[keys [i * nprobe + ik]]->rdlock();
					//std::cout << "rdlock\t" << keys [i * nprobe + ik] << std::endl;
			    }
            	ndis += scan_one_list
                (keys [i * nprobe + ik],
				...
				if (toBlock) {
					bucketRwLock[keys [i * nprobe + ik]]->unlock();
					//std::cout << "rdunlock\t" << keys [i * nprobe + ik] << std::endl;
			    }
            }
