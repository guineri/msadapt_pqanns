...
void merge(int k, float **dists, faiss::Index::idx_t **labels, int n_threads, 
		   float *dist_resp, faiss::Index::idx_t * label_resp) {
    int counter[n_threads];
    for (int i = 0; i < n_threads; i++) counter[i] = 0;

    for (int topi = 0; topi < k; topi++) {
        float bestDist = UINT_MAX;
        long bestId = -1;
        int fromShard = -1;

        for (int shard = 0; shard < n_threads; shard++) {
            if (counter[shard] == k) continue;

            if (dists[shard][counter[shard]] < bestDist) {
                bestDist = dists[shard][counter[shard]];
                bestId = labels[shard][counter[shard]];
                fromShard = shard;
            }
        }

        label_resp[topi] = bestId;
		dist_resp[topi] = bestDist;
        counter[fromShard]++;
    }
}


void IndexFlat::search_parallel (idx_t n, const float *x, idx_t k,
                               float *distances, idx_t *labels, int n_threads) const
{
	idx_t **labels_local = (idx_t**) malloc (n_threads * sizeof(idx_t*));
	float **distances_local = (float**) malloc (n_threads * sizeof(float*));

	for (int t = 0; t < n_threads; t++) {
		labels_local[t] = (idx_t*) malloc (k * sizeof(idx_t));
		distances_local[t] = (float*) malloc (k * sizeof(float));
	}
	...
	#pragma omp parallel
	{
		...
		int omp_threads_n = omp_get_num_threads();

		long data_amount_per_thread = (xb.size() / d) / omp_threads_n;

		int start_offset = thread_rank * data_amount_per_thread;
		int end_offset = (start_offset + data_amount_per_thread) - 1;
		if (thread_rank == (omp_get_num_threads() - 1) ) {
			end_offset = (xb.size() / d) - 1;
		}
			
		if (metric_type == METRIC_INNER_PRODUCT) {
			float_minheap_array_t res_local = {
        		size_t(n), size_t(k), labels_local[thread_rank], distances_local[thread_rank]};
        	knn_inner_product (x, xb.data() + start_offset*d, d, n, data_amount_per_thread, &res_local);
    	} else if (metric_type == METRIC_L2) {
			float_maxheap_array_t res_local = {
        		size_t(n), size_t(k), labels_local[thread_rank], distances_local[thread_rank]};
        	knn_L2sqr (x, xb.data() + start_offset*d, d, n, data_amount_per_thread, &res_local);
    	} else {
			float_maxheap_array_t res_local = {
        		size_t(n), size_t(k), labels_local[thread_rank], distances_local[thread_rank]};
        	knn_extra_metrics (x, xb.data() + start_offset*d, d, n, data_amount_per_thread,
                           	   metric_type, metric_arg,
                           	   &res_local);
    	}

		for(int i=0; i < k; i++) {
			labels_local[thread_rank][i] += start_offset;
		}
	}

	merge(k, distances_local, labels_local, n_threads, distances, labels);
}
