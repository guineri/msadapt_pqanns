...
// Clean Constructor
template <typename T>
QSMatrix<T>::QSMatrix(unsigned _rows, unsigned _cols)
{
    mat.resize(_rows);
    for (unsigned i = 0; i < mat.size(); i++) {
        mat[i].resize(_cols);
...
// Fill Matrix
template <typename T>
void QSMatrix<T>::fillRandomly()
{
    for (unsigned i = 0; i < rows; i++) {
        for (unsigned j = 0; j < cols; j++) {
            mat[i][j] = (T)rand() / RAND_MAX;
        }
    }
}
...
// Get the number of columns of the matrix
template <typename T>
unsigned QSMatrix<T>::dimension() const
{
    return this->cols;
}
...
// Get the number of columns of the matrix
template <typename T>
unsigned QSMatrix<T>::size() const
{
    return this->rows;
}
...
// Print head
template <typename T>
void QSMatrix<T>::head(unsigned count)
{
    unsigned printed = 0;
    for (unsigned i = 0; i < rows; i++) {
        for (unsigned j = 0; j < cols; j++) {
...
template <typename T>
std::vector<T> QSMatrix<T>::getRow(int index)
{
    return mat[index];
}
#endif